...
public class TierlistManager {
    ...
    private void loadFromConfig() {
        for (String[] entry : DEFAULT_TIERLISTS) {
            ...
            loadGamemodes(tierlist, id);
            
            tierlists.put(id, tierlist);
        }
    }

    private void loadGamemodes(Tierlist tierlist, String tierlistKey) {
        ...
        for (String gamemodeKey : kac.getGamemodeKeys(tierlistKey)) {
            ...
            int slot = kac.getGamemodeSlot(tierlistKey, gamemodeKey);
            String queueType = kac.getGamemodeQueueType(tierlistKey, gamemodeKey);
            
            Gamemode gamemode = new Gamemode(gamemodeKey, name, material, fullId, slot, queueType);
            tierlist.addGamemode(gamemode);
        }
    }
