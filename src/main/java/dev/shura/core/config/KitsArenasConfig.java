...
public class KitsArenasConfig {
    ...
    public void load() {
        file = new File(plugin.getDataFolder(), "kits-arenas.yml");
        if (!file.exists()) {
            plugin.saveResource("kits-arenas.yml", false);
        }
    ...
    public void save() {
        try {
            ...
            plugin.getLogger().severe("Failed to save kits-arenas.yml: " + e.getMessage());
        }
    }
