...
public class KitManager {
    ...
    public void saveKit(Kit kit) {
        ...
        plugin.getDatabaseService().updateAsync(
                "INSERT INTO kits (id, name, tierlist_id, inventory, armor, effects, rules, created_at) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET " +
                "name=excluded.name, tierlist_id=excluded.tierlist_id, inventory=excluded.inventory, " +
                "armor=excluded.armor, effects=excluded.effects, rules=excluded.rules",
                stmt -> {
                    ...
                    stmt.setString(6, effectsJson);
                    stmt.setString(7, rulesJson);
    ...
    private Kit deserialize(ResultSet rs) throws SQLException {
        ...
        String offhandJson = rs.getString("offhand");
