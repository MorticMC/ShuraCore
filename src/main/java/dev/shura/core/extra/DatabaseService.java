...
public class DatabaseService {
    ...
    private void createTables() throws SQLException {
        try (Connection conn = getConnection(); Statement stmt = conn.createStatement()) {
            ...
            // Migration: Add offhand column if it doesn't exist
            try {
                stmt.executeUpdate("ALTER TABLE kits ADD COLUMN offhand TEXT NOT NULL DEFAULT '[]'");
            ...
            stmt.executeUpdate("""
                CREATE TABLE IF NOT EXISTS arenas (
                    ...
                    bound_kits TEXT NOT NULL DEFAULT ''
                )
            """);
            
            try {
                stmt.executeUpdate("ALTER TABLE arenas ADD COLUMN bound_kits TEXT NOT NULL DEFAULT ''");
            } catch (SQLException e) {
                // Column already exists
            }
            
            try {
                stmt.executeUpdate("ALTER TABLE arenas ADD COLUMN auto_clone_count INTEGER NOT NULL DEFAULT 2");
            } catch (SQLException e) {
                // Column already exists
            }
