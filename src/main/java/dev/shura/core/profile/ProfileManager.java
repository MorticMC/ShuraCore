...
public class ProfileManager {
    ...
    private CompletableFuture<Profile> createProfile(UUID uuid, String name) {
        ...
        cache.put(uuid, profile);
        return db.update("INSERT INTO profiles (uuid, name, tierlist_matches, stats, last_seen) VALUES (?, ?, '{}', '{}', ?)",
                stmt -> {
    ...
    public void saveProfile(Profile profile) {
        ...
        db.updateAsync("UPDATE profiles SET name = ?, tierlist_matches = ?, stats = ?, last_seen = ? WHERE uuid = ?",
                stmt -> {
                    stmt.setString(1, profile.getName());
                    stmt.setString(2, matchesJson);
                    stmt.setString(3, statsJson);
    ...
    public CompletableFuture<List<Profile>> getTopByTierlist(String tierlistId, int limit) {
        return db.query("SELECT * FROM profiles ORDER BY JSON_EXTRACT(tierlist_matches, '$." + tierlistId + "') DESC LIMIT ?",
